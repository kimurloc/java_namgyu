...
SELECT p.member_id,
CASE
	WHEN DATEDIFF(NOW(), p.created_at) <= 1 THEN CONCAT('(NEW)', title)
	WHEN count(r.id) > 3 THEN CONCAT('(Best)', title)
	ELSE title
END
content, p.created_at, count(r.id)
FROM post as p
LEFT JOIN reply AS r ON p.id = r.post_id
GROUP BY p.id, p.member_id, title, p.created_at
ORDER BY p.created_at DESC;

-- 2. 게시글 상세 조회 (게시글 제목, 내용, 작성자 이름, 이메일, 댓글 작성자 이름, 댓글 내용 조회)
SELECT p.member_id, title, p.content, r.member_id, r.content AS reply
FROM post AS p
RIGHT JOIN reply AS r ON p.id = r.post_id;

-- 3. 게시글 등록 (새로운 게시글 작성)
INSERT INTO post (member_id, title, content)
VALUES (1+RAND()*5, '새로운 게시글', '새로운 게시글 내용');
...
UPDATE post
SET title = '수정된 두번째 게시글', content = '수정이 반영된 본문'
WHERE id = 2
...
SELECT *
FROM post
WHERE title OR content LIKE '%자바%';
